...
public class WebServiceDispatcher extends DestinationConnector {
	 ...
	 */
	private Set<CloseableHttpClient> clients = Collections
			.newSetFromMap(new ConcurrentHashMap<CloseableHttpClient, Boolean>());
		...
		if (executor != null) {
			...
			if (!shutdown) {
				try {
					...
					throw new ConnectorTaskException("Halt task interrupted while waiting for executor to shutdown.",
							e);
				}
				...
				if (numTasks > 0) {
					...
					eventController.dispatchEvent(
							new ErrorEvent(getChannelId(), getMetaDataId(), null, ErrorEventType.DESTINATION_CONNECTOR,
									getDestinationName(), getConnectorProperties().getName(), message, null));
				}
			}
		}
	...
	private void createDispatch(WebServiceDispatcherProperties webServiceDispatcherProperties,
			DispatchContainer dispatchContainer, int timeout) throws Exception {
		...
		/*
		 * The dispatch needs to be created if it hasn't been created yet (null). It
		 * needs to be recreated if any of the above variables are different than what
		 * were used to create the current dispatch object. This could happen if
		 * variables are being used for these properties.
		 */
		if (dispatchContainer.getDispatch() == null
			...
			logger.debug("Creating web service: url=" + endpointUrl.toString() + ", service=" + serviceQName + ", port="
					+ portQName);
			Service service = Service.create(endpointUrl, serviceQName);

			Dispatch<SOAPMessage> dispatch = service.createDispatch(portQName, SOAPMessage.class, Service.Mode.MESSAGE);
			dispatch.getRequestContext().put("com.sun.xml.internal.ws.transport.https.client.SSLSocketFactory", SSLUtils.getATNAContext().getSocketFactory());
			dispatch.getRequestContext().put("com.sun.xml.ws.transport.https.client.SSLSocketFactory", SSLUtils.getATNAContext().getSocketFactory()); // JAX-WS RI
			if (timeout > 0) {
	...
	/**
	 * Returns the URL for the passed in String. If the URL requires authentication,
	 * then the WSDL is saved as a temp file and the URL for that file is returned.
	 * 
	 * @param wsdlUrl
	 * @param username
	 * @param password
	 ...
	 * @throws Exception
	 */
	protected URL getWsdlUrl(WebServiceDispatcherProperties webServiceDispatcherProperties,
			DispatchContainer dispatchContainer, int timeout) throws Exception {
		...
		if (!scheme.equalsIgnoreCase("file") || webServiceDispatcherProperties.getLocationURI().toLowerCase().startsWith("https")) {
			...
			/**
			 * If call is HTTPS changing the SSL Context to our project SSL
			 */
			...
			try {
				...
				if (dispatchContainer.getCurrentUsername() != null && dispatchContainer.getCurrentPassword() != null) {
					...
					Credentials credentials = new UsernamePasswordCredentials(dispatchContainer.getCurrentUsername(),
							dispatchContainer.getCurrentPassword());
	...
	public void replaceConnectorProperties(ConnectorProperties connectorProperties, ConnectorMessage connectorMessage) {
		...
		webServiceDispatcherProperties
				.setWsdlUrl(replacer.replaceValues(webServiceDispatcherProperties.getWsdlUrl(), connectorMessage));
		webServiceDispatcherProperties
				.setUsername(replacer.replaceValues(webServiceDispatcherProperties.getUsername(), connectorMessage));
		webServiceDispatcherProperties
				.setPassword(replacer.replaceValues(webServiceDispatcherProperties.getPassword(), connectorMessage));
		webServiceDispatcherProperties
				.setService(replacer.replaceValues(webServiceDispatcherProperties.getService(), connectorMessage));
		webServiceDispatcherProperties
				.setPort(replacer.replaceValues(webServiceDispatcherProperties.getPort(), connectorMessage));
		webServiceDispatcherProperties.setLocationURI(
				replacer.replaceValues(webServiceDispatcherProperties.getLocationURI(), connectorMessage));
		webServiceDispatcherProperties.setSocketTimeout(
				replacer.replaceValues(webServiceDispatcherProperties.getSocketTimeout(), connectorMessage));

		webServiceDispatcherProperties.setSoapAction(
				replacer.replaceValues(webServiceDispatcherProperties.getSoapAction(), connectorMessage));
		webServiceDispatcherProperties
				.setEnvelope(replacer.replaceValues(webServiceDispatcherProperties.getEnvelope(), connectorMessage));
		...
		webServiceDispatcherProperties.setHeadersVariable(
				replacer.replaceValues(webServiceDispatcherProperties.getHeadersVariable(), connectorMessage));
		...
		if (webServiceDispatcherProperties.isUseMtom()) {
			...
			webServiceDispatcherProperties.setAttachmentsVariable(
					replacer.replaceValues(webServiceDispatcherProperties.getAttachmentsVariable(), connectorMessage));

		}
	}

//	public boolean isATNARequest = false;

	@Override
	public Response send(ConnectorProperties connectorProperties, ConnectorMessage connectorMessage) {
		WebServiceDispatcherProperties webServiceDispatcherProperties = (WebServiceDispatcherProperties) connectorProperties;

//		if (connectorMessage.getChannelId().equals("e8ba57a9-71a3-46e5-abb1-2dd4782eb571"))
//			isATNARequest = true;
		...
		try {
			...
			/*
			 * Initialize the dispatch object if it hasn't been initialized yet, or create a
			 * new one if the connector properties have changed due to variables.
			 */
			...
			String content = attachmentHandlerProvider.reAttachMessage(webServiceDispatcherProperties.getEnvelope(),
					connectorMessage,
					webServiceDispatcherProperties.getDestinationConnectorProperties().isReattachAttachments());
			...
			if (webServiceDispatcherProperties.isUseMtom()) {
				...
				List<AttachmentEntry> attachmentEntries = getAttachments(webServiceDispatcherProperties,
						connectorMessage);
				...
				for (AttachmentEntry entry : attachmentEntries) {
					...
					String attachmentContent = attachmentHandlerProvider.reAttachMessage(entry.getContent(),
							connectorMessage,
							webServiceDispatcherProperties.getDestinationConnectorProperties().isReattachAttachments());
			...
			/*
			 * Attempt the invocation until we hit the maximum allowed redirects. The
			 * redirections we handle are when the scheme changes (i.e. from HTTP to HTTPS).
			 */
			do {
				redirect = false;
				tryCount++;
				...
				try {
					...
					// Automatically accept message; if a SOAP fault occurred an exception will have
					// been thrown
					...
					if (dispatch.getResponseContext() != null) {
						...
						Map<String, List<String>> headers = (Map<String, List<String>>) dispatch.getResponseContext()
					...
					if (tryCount < MAX_REDIRECTS && responseCode != null && responseCode >= 300 && responseCode < 400
						...
						if (e instanceof NoRouteToHostException
							...
							eventController.dispatchEvent(new ErrorEvent(getChannelId(), getMetaDataId(),
									...
									getDestinationName(), connectorProperties.getName(), "HTTP transport error.", e));
							...
							responseStatusMessage = ErrorMessageBuilder.buildErrorResponse("Error invoking web service",
									e);
							responseError = ErrorMessageBuilder.buildErrorMessage(connectorProperties.getName(),
	...
	protected void handleSOAPResult(ConnectorProperties connectorProperties, ConnectorMessage connectorMessage,
			SOAPMessage result) throws Exception {
	}
	...
	Map<String, List<String>> getHeaders(WebServiceDispatcherProperties webServiceDispatcherProperties,
			ConnectorMessage connectorMessage) {
		return HttpUtil.getTableMap(webServiceDispatcherProperties.isUseHeadersVariable(),
				webServiceDispatcherProperties.getHeadersVariable(), webServiceDispatcherProperties.getHeadersMap(),
				getMessageMaps(), connectorMessage);
	}

	List<AttachmentEntry> getAttachments(WebServiceDispatcherProperties webServiceDispatcherProperties,
			ConnectorMessage connectorMessage) {
		List<AttachmentEntry> attachmentList = new ArrayList<>();

		if (webServiceDispatcherProperties.isUseAttachmentsVariable()) {
			try {
				List<?> attachmentEntries = (List<?>) getMessageMaps()
						.get(webServiceDispatcherProperties.getAttachmentsVariable(), connectorMessage);
				if (attachmentEntries != null) {
					for (Object entry : attachmentEntries) {
						if (entry instanceof AttachmentEntry) {
							...
							logger.trace("Error getting attachment entry from map '"
									+ webServiceDispatcherProperties.getAttachmentsVariable() + "'. Skipping entry.");
						}
					}
				} else {
					logger.warn("Attachments list variable '" + webServiceDispatcherProperties.getAttachmentsVariable()
							+ "' not found.");
				}
			} catch (Exception e) {
				logger.warn("Error getting attachments from map '" + webServiceDispatcherProperties.getHeadersVariable()
						+ "'.", e);
			}
		} else {
			...
			List<String> attachmentContents = new ArrayList<String>(
					webServiceDispatcherProperties.getAttachmentContents());
			List<String> attachmentTypes = new ArrayList<String>(webServiceDispatcherProperties.getAttachmentTypes());

			for (int i = 0; i < attachmentIds.size(); i++) {
				attachmentList.add(
						new AttachmentEntry(attachmentIds.get(i), attachmentContents.get(i), attachmentTypes.get(i)));
			}
		}
